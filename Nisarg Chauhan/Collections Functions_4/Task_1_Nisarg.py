int_list=[]
str_list=[]

#Taking input from user for list 1 
a=int(input("Enter the number of elements you want to add in list 1 (int_list): "))

for i in range(a):
    int_list.append(int(input("Enter an integer: ")))

#Taking input from user for list 2
b=int(input("Enter the number of elements you want to add in list 2 (str_list): "))

for i in range(b):
    str_list.append(input("Enter a string: "))

#Printing List 1 and List 2
print("List 1 (int_list) is: ", int_list)
print("List 2 (str_list) is: ", str_list)

#Finding the maximum and minimum value in List 1 (int_list)
print("Max of List 1 is: ", max(int_list))
print("Min of List 1 is: ", min(int_list))

print("Reversed List 2 is: ", list(reversed(str_list)))
